...
std::string format_row(const std::vector<std::string>& row,
...
void CSVGuesser::Guesser::bad_row_handler(std::vector<std::string> record) {
    ...
    if (row_tally.find(record.size()) != row_tally.end()) {
        row_tally[record.size()]++;
    }
    else {
 ...
 *
 */
std::vector<std::string> get_col_names(const std::string& filename,
                                       CSVFormat format) {
